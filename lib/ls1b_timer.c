 ...
 * @ret 基地址
 */
unsigned int timer_get_reg_base(ls1b_timer_t timer)
{
...
void timer_init(timer_info_t *timer_info)
{
    ...
    reg_write_32(0,                     (volatile unsigned int *)(timer_reg_base + LS1B_PWM_HRC));
    reg_write_32(--tmp,                 (volatile unsigned int *)(timer_reg_base + LS1B_PWM_LRC));
    reg_write_32(0,                     (volatile unsigned int *)(timer_reg_base + LS1B_PWM_CNTR));
    reg_write_32(ctrl,                  (volatile unsigned int *)(timer_reg_base + LS1B_PWM_CTRL));
...
BOOL timer_is_time_out(timer_info_t *timer_info)
{
    ...
    ctrl = reg_read_32((volatile unsigned int *)(timer_reg_base + LS1B_PWM_CTRL));

    // 判断中断状态位
    if (ctrl & (1 << LS1B_PWM_INT_SR))
    {
        return TRUE;
    }
    else
...
void timer_stop(timer_info_t *timer_info)
{
    ...
    timer_reg_base = timer_get_reg_base(timer_info->timer);
    reg_write_32(0, (volatile unsigned int *)(timer_reg_base + LS1B_PWM_CTRL));
...
unsigned long timer_get_time_ns(timer_info_t *timer_info)
{
    ...
    // 读取寄存器CNTR的值
    timer_reg_base = timer_get_reg_base(timer_info->timer);
    cntr = reg_read_32((volatile unsigned int *)(timer_reg_base + LS1B_PWM_CNTR));
...
void timer_print_regs(timer_info_t *timer_info)
{
    ...
    myprintf("CNTR=0x%x, HRC=0x%x, LRC=0x%x, CTRL=0x%x\n",
              reg_read_32((volatile unsigned int *)(timer_reg_base + LS1B_PWM_CNTR)),
              reg_read_32((volatile unsigned int *)(timer_reg_base + LS1B_PWM_HRC)),
              reg_read_32((volatile unsigned int *)(timer_reg_base + LS1B_PWM_LRC)),
              reg_read_32((volatile unsigned int *)(timer_reg_base + LS1B_PWM_CTRL)));
