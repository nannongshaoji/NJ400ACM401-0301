...
// 寄存器偏移
#define LS1B_SPI_SPCR_OFFSET                (0)     // 控制寄存器
#define LS1B_SPI_SPSR_OFFSET                (1)     // 状态寄存器
#define LS1B_SPI_TxFIFO_OFFSET              (2)     // 发送的数据寄存器，与接收数据寄存器的偏移相同
#define LS1B_SPI_RxFIFO_OFFSET              (2)     // 接收的数据寄存器，与发送数据寄存器的偏移相同
#define LS1B_SPI_SPER_OFFSET                (3)     // 外部寄存器
#define LS1B_SPI_SFC_PARAM_OFFSET           (4)     // 参数控制寄存器
#define LS1B_SPI_SFC_SOFTCS_OFFSET          (5)     // 片选控制寄存器
#define LS1B_SPI_SFC_TIMING_OFFSET          (6)     // 时序控制寄存器

// 寄存器SPCR中的位域
#define LS1B_SPI_SPCR_SPIE_BIT              (7)								//中断输出使能位，高有效
#define LS1B_SPI_SPCR_SPIE_MASK             (0x01 << LS1B_SPI_SPCR_SPIE_BIT)
#define LS1B_SPI_SPCR_SPE_BIT               (6)								//SPI使能位，高有效
#define LS1B_SPI_SPCR_SPE_MASK              (0x01 << LS1B_SPI_SPCR_SPE_BIT)
#define LS1B_SPI_SPCR_CPOL_BIT              (3)								//时钟极性位
#define LS1B_SPI_SPCR_CPOL_MASK             (0x01 << LS1B_SPI_SPCR_CPOL_BIT)
#define LS1B_SPI_SPCR_CPHA_BIT              (2)								//时钟相位位
#define LS1B_SPI_SPCR_CPHA_MASK             (0x01 << LS1B_SPI_SPCR_CPHA_BIT)
#define LS1B_SPI_SPCR_SPR_BIT               (0)								//分频配置
#define LS1B_SPI_SPCR_SPR_MASK              (0x03 << LS1B_SPI_SPCR_SPR_BIT)

// 寄存器SPSR中的位域
#define LS1B_SPI_SPSR_SPIF_BIT              (7)								//中断标志位
#define LS1B_SPI_SPSR_SPIF_MASK             (0x01 << LS1B_SPI_SPSR_SPIF_BIT)
#define LS1B_SPI_SPSR_WCOL_BIT              (6)								//写寄存器溢出标志位
#define LS1B_SPI_SPSR_WCOL_MASK             (0x01 << LS1B_SPI_SPSR_WCOL_BIT)

// 寄存器SPER中的位域
#define LS1B_SPI_SPER_SPRE_BIT              (0)								//分频配置
#define LS1B_SPI_SPER_SPRE_MASK             (0x3 << LS1B_SPI_SPER_SPRE_BIT)

// 寄存器SFC_SOFTCS的位域
#define LS1B_SPI_SFC_SOFTCS_CSN_BIT         (4)										//csn引脚输出值
#define LS1B_SPI_SFC_SOFTCS_CSN_MASK        (0x0f << LS1B_SPI_SFC_SOFTCS_CSN_BIT)
#define LS1B_SPI_SFC_SOFTCS_CSEN_BIT        (0)										//can引脚使能位
...
void spi_set_clock(ls1b_spi_info_t *spi_info_p)
{
    ...
    val &= (~LS1B_SPI_SPCR_SPR_MASK);                       // spr清零
    val |= (div & LS1B_SPI_SPCR_SPR_MASK);                  // 设置新的spr
    reg_write_8(val, spi_base + LS1B_SPI_SPCR_OFFSET);

    // 设置spre
    val = reg_read_8(spi_base + LS1B_SPI_SPER_OFFSET);
    val &= (~LS1B_SPI_SPER_SPRE_MASK);                      // spre清零
    val |= ((div >> 2) & LS1B_SPI_SPER_SPRE_MASK);        	// 设置新的spre
...
void spi_set_mode(ls1b_spi_info_t *spi_info_p)
{
    ...
    // 设置时钟极性--cpol
    val &= (~LS1B_SPI_SPCR_CPOL_MASK);                      // cpol清0
    val |= (spi_info_p->cpol << LS1B_SPI_SPCR_CPOL_BIT);    // 写入新的cpol
    
    // 设置时钟相位--cpha
    val &= (~LS1B_SPI_SPCR_CPHA_MASK);                      // cpha清0
    val |= (spi_info_p->cpha << LS1B_SPI_SPCR_CPHA_BIT);    // 写入新的cpha
...
void spi_init(ls1b_spi_info_t *spi_info_p)
{
    ...
    reg_write_8(0x03, spi_base + LS1B_SPI_SPER_OFFSET);//0x03同时 0x07错开半周期
...
inline void spi_clear(ls1b_spi_info_t *spi_info_p)
{
    ...
    if (LS1B_SPI_SPSR_WCOL_MASK & val)
    {
        ...
        reg_write_8(val & ~LS1B_SPI_SPSR_WCOL_MASK, spi_base + LS1B_SPI_SPSR_OFFSET);   // 写0，linux源码中是写0
		//reg_write_8(val | LS1B_SPI_SPSR_WCOL_MASK, spi_base + LS1B_SPI_SPSR_OFFSET);  // 写1，按照1c手册，应该写1
    }
...
unsigned char spi_txrx_byte(ls1b_spi_info_t *spi_info_p, unsigned char tx_ch)
{
    ...
    // 收发数据
    reg_write_8(tx_ch, spi_base + LS1B_SPI_TxFIFO_OFFSET);      // 开始发送
    spi_wait_txrx_done(spi_info_p);                             // 等待收发完成
    rx_ch = reg_read_8(spi_base + LS1B_SPI_RxFIFO_OFFSET);      // 读取收到的数据
